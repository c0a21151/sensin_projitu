...
@WebServlet("/ex06/LoginCookieServlet")
public class LoginCookieServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (cnt == 0) {
				// 見つからない == 認証失敗
				forwardURL = "/ex06/cookiefailure.jsp";
			} else {
				// 最初の要素をuseridとする．
				String uid = nameArray.get(0);
				request.setAttribute("userid", uid);
				//クッキーをresponseにセットする．
				Cookie cookie = new Cookie("cpassword", password);
				//30日間有効なcookieとする．
				cookie.setMaxAge(60 * 60 * 24 * 30);
				//Responseにcookieを設定する．
				response.addCookie(cookie);
				
				forwardURL = "/ex06/cookiesuccess.jsp";
